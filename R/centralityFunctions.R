...
#   require(igraph)
  ...
  # compute centrality with package cigraph: InDegree, OutDegree, Closeness, Betwenness
  net_qg<-cigraph(x, diag=FALSE, labels=colnames(x), DoNotPlot=TRUE, minimum=0)
  ...
  if(sum(largcomp)<ncol(x) & sum(largcomp) > 1)
  {
    ...
    clos<-centrality(cigraph(x2, diag=FALSE, labels=colnames(x)[largcomp], DoNotPlot=TRUE, minimum=0))$Closeness
