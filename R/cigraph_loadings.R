...
cigraph.loadings=function( fact, ...)
{
...
# Output arguments:
if(is.null(arguments$filetype)) filetype="default" else filetype=arguments$filetype
if(is.null(arguments$filename)) filename="cigraph" else filename=arguments$filename
...
Q <- do.call(cigraph,args)

# 
# Q <- cigraph(m,layout=l,vsize=Gvsize,color=Gcolor,labels=Glabels,shape=shape,filetype="",curve=curve,
...
else if (legend & filetype!="pdf") warning("Legend in cigraph.loadings only supported for pdf output")
