...
ggmFit <- function(
  pcor, # pcor matrix or cigraph object
  ...
  if (refit){
    ...
    if (missing(invSigma)){
      # If cigraph object, extract:
      if (is(pcor,"cigraph")){
        pcor <- cigraph::getWmat(pcor)
        diag(pcor) <- 1
      }
  ...
  # Results object:
  Results <- list(
    network = cigraph::wi2net(invSigma),
    invSigma=invSigma,
