...
centrality <- function(graph,alpha=1,posfun=abs,pkg = c("igraph","cigraph"),all.shortest.paths=FALSE,
                       weighted = TRUE, signed = TRUE, R2 = FALSE)
{

  # Check for correct class:
  #   if (class(graph) != "cigraph") stop("Must be a 'cigraph' object")
  ...
  pkg <- match.arg(pkg)
#   if (missing(pkg)){
#     pkg <- ifelse(all(W==t(W)),"igraph","cigraph")
#     
#   }
