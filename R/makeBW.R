...
{
  # convert a color cigraph in a black and white version
  # x is a cigraph object
  ...
  # generate 12  combinations of density & angles as colors are in the original cigraph
  density <- c(10, 20, 30)
  ...
  N <- length(unq) 
  if(N > 12)
    stop("Too many colors: Black & White cigraphs can be plotted with up to 14 colors")
...
#   x$graphAttributes$Nodes$label.color <- lblcol
  ...
  # plot and return the cigraph object
  if(plot) plot(x)
  invisible(x)
}