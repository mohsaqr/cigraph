flow <- function(
  ...
  # Test input:
  if (!is(object,"cigraph")){
    warning("Input is not a cigraph object, runnin gcigraph")
    object <- cigraph(object, ..., DoNotPlot = TRUE)
  }
  if (length(from)!=1){
  ...
  # 
  # cigraph(VeinEdgelist[,1:3],layout=Layout)
  # cigraph(E[,1:3],layout=Layout, directed = FALSE,
  #        curve = Curve)
  # 
  #ECPs:
  ...
  # Plot:
  # cigraph(object, layout = Layout, curve = Curve, edgeConnectPoints = ECP, curveScale = FALSE,fade=fade,labels=labels, ...)
  
  
  cigraph(as.matrix(E[,1:3]), layout = Layout, 
         curve = Curve, 
